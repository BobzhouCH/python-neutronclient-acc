...
class CreateFloatingIP(neutronV20.CreateCommand):
    ...
    def add_known_arguments(self, parser):
        ...
        parser.add_argument(
           '--bandwidth',
            help=_('Bandwidth of the floating IP'))
        parser.add_argument(
            '--subnet',
            dest='subnet_id',
...
class UpdateBandwidthFloatingIP(neutronV20.NeutronCommand):
    """Update bandwidth of floating ip"""
    api = 'network'
    resource = 'floatingip'

    def get_parser(self, prog_name):
        parser = super(UpdateBandwidthFloatingIP, self).get_parser(prog_name)
        parser.add_argument(
            'floatingip_id', metavar='FLOATINGIP_ID',
            help=_('ID of the floating IP to update'))
        parser.add_argument(
            '--bandwidth',
            help=_('New bandwidth of the floating IP'))
        return parser
    ...
    def take_action(self, parsed_args):
        ...
        update_dict = {}
        update_dict['bandwidth'] = parsed_args.bandwidth
        neutron_client.update_floatingip(parsed_args.floatingip_id,
                                         {'floatingip': update_dict})
        print(_('Updated bandwidth of floating IP %s') % parsed_args.floatingip_id,
